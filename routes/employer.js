...

{
    ...
    //         const updatEmployer = await pool.query(`
    //     UPDATE employer SET name = $1, dagree = $2, salary = $3, job_id = $4, WHERE id = $5 RETURNING *
    //     `, [
    //             name ? name : oldEmployer.rows[0].name,
    //             dagree ? dagree : oldEmployer.rows[0].dagree,
    //             salary ? salary : oldEmployer.rows[0].salary,
    //             job_id ? job_id : oldEmployer.rows[0].job_id,
    //             id])

    //         res.status(201).json(updatEmployer.rows[0])
